...
module Bulkrax
  ##
  # Validates file references against zip archive contents.
  class FileValidator
    ...
    def referenced_files
      @referenced_files ||= @csv_data.flat_map do |item|
        ...
        Array(item[:file]).flat_map do |value|
          value.to_s.split(Bulkrax::CsvParser.file_split_pattern).map { |f| File.basename(f.strip) }
        end
      end.compact
    end
