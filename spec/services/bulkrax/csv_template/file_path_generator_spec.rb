  ...
  describe '.build_filename' do
    ...
    context 'with both context and tenant' do
      it 'includes both in filename' do
        filename = described_class.build_filename('work', 'tenant_xyz')
        expect(filename).to eq("#{described_class::TEMPLATE_PREFIX}_context-work_tenant-tenant_xyz_#{timestamp}.csv")
      end
    end
  end
